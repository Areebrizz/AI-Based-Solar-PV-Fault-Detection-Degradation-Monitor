...
> ⚠️ Important: All performance results shown are obtained from synthetic and simulated operational scenarios, not long-term field deployments.

---

## 📊 Live Demo

Interactive Streamlit Application badge:
![Streamlit Demo](https://static.streamlit.io/badges/streamlit_badge_black_white.svg)
...
Example formulas used in the project (simplified):
```python
...
> ⚠️ Known limitation: Rule-based logic is brittle across technologies and climates.
...
Results below are from synthetic and simulated operational datasets, used for validation of system logic — not field-validated claims.
...
Example degradation estimation:
```text
...
## 🔍 Explainability Roadmap (Critical for Safety-Critical Systems)

Current version focuses on detection and diagnosis. Planned explainability extensions:
...
### Run (example)
```bash
# if repository main app is app.py or specific filename, adjust accordingly:
streamlit run app.py
```

---

## 🐳 Deployment (Prototype-Grade Dockerfile)
```dockerfile
...
> Note: Streamlit is used intentionally as a deployable analytics prototype, not a hardened industrial SCADA replacement.
